...
# import keyring  # 未利用のため、コメントアウト
import requests

# import chromedriver_binary  # 未利用のため、コメントアウト
...
parser.add_argument("-p", "--password", required=True)  # 引数でのパスワード入力を必須とする
parser.add_argument("-c", "--mf_cookies", default="mf_cookies.pkl")
args = parser.parse_args()

username = args.username
password = args.password
# if not password:  # 常に引数でパスワードを与えるようにするため、コメントアウト
...
driver = webdriver.Remote(  # 自分の環境用に変更
...
with requests.session() as s:
    ...
    authorize_response = s.get(
        authorize_url, params=authorize_params, allow_redirects=False
    )
