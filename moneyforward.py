...
# import datetime  # 別途import文があるため、コメントアウト
import json
import logging
import os
import pickle
...
# import sqlalchemy  # 未利用のため、コメントアウト
...
def save_large_categories_csv(fn, large_categories):
    with open(fn, "wt", encoding="utf_8_sig") as f:
        ...
        writer.writerow(
            "large_category_id large_category_name middle_category_id middle_category_name user_category".split()
        )
...
def upsert(frame, name: str, unique_index_label, con):
    ...
    def _execute_insert(self, conn, keys, data_iter):
        ...
        insert_statement = f"INSERT OR REPLACE INTO {escape(self.name)} ({col_names}) VALUES ({wildcards})"
...
def get_large_categories(s, args):
    ...
    if args.sqlite:
        ...
        for large_category in large_categories:
            large_category_list.append(
                dict(id=large_category["id"], name=large_category["name"])
            )
            for middle_category in large_category["middle_categories"]:
                middle_category_list.append(
                    dict(
                        large_category_id=large_category["id"],
...
def request_service_detail(s, args):
    ...
    service_detail = s.get(
        "https://moneyforward.com/sp/service_detail/{}".format(args.account_id_hash),
        params=params,
    )
...
def request_accounts(s, args):
    ...
    accounts = s.get(
        "https://moneyforward.com/sp2/accounts/{}".format(args.id), params=params
    )
...
def request_cf_sum_by_sub_account(s, args):
    ...
    cf_sum_by_sub_account = s.get(
        "https://moneyforward.com/sp/cf_sum_by_sub_account", params=params
    )
...
def request_cf_term_data_by_sub_account(
    s, sub_account_id_hash, date_from=None, date_to=None
):
    ...
    cf_term_data_by_sub_account = s.get(
        "https://moneyforward.com/sp/cf_term_data_by_sub_account", params=params
    )
...
def get_term_data_list(cf_term_data_by_sub_account, s=None, large=None, middle=None):
    ...
    if large and middle:
        for i in range(len(user_asset_acts)):
            user_asset_acts[i]["large_category"] = large[
                user_asset_acts[i]["large_category_id"]
            ]
            user_asset_acts[i]["middle_category"] = middle[
                user_asset_acts[i]["middle_category_id"]
            ]

    for i, e in enumerate(user_asset_acts):
        user_asset_acts[i]["date"] = datetime.fromisoformat(
            e["recognized_at"]
        ).strftime("%y/%m/%d")
        user_asset_acts[i]["year"] = datetime.fromisoformat(
            e["recognized_at"]
        ).strftime("CY%y")
        user_asset_acts[i]["month"] = datetime.fromisoformat(
            e["recognized_at"]
        ).strftime("%y'%m")
...
def get_term_data_by_sub_account(s, args):
    cf_term_data_by_sub_account = request_cf_term_data_by_sub_account(
        s, args.sub_account_id_hash, args.date_from, args.date_to
    )
    if args.json:
...
def request_term_data(s, args):
    ...
    def gen_prams(sub_account_id_hash_list, args):
        for sub_account_id_hash in tqdm(sub_account_id_hash_list, desc="sub_account"):
            ...
            date_from_list = list(
                xrange(args.date_from, args.date_to, timedelta(days=365))
            )
...
def get_term_data(s, args):
    ...
    if args.sqlite:
        if args.sqlite_header:
            ...
            rename_header = dict(
                x.split("=", 2) for x in args.sqlite_header if x.find("=") != -1
            )
            if rename_header:
...
def add_dummy_data_to_user_asset_act(s, args):
    with closing(sqlite3.connect(args.sqlite)) as con:
        df = pd.read_sql(
            "SELECT * FROM user_asset_act WHERE id > 0 AND content = ?",
            con,
            params=(args.content,),
        )
...
def add_dummy_offset_data_to_user_asset_act(s, args):
    with closing(sqlite3.connect(args.sqlite)) as con:
        ...
        df = pd.read_sql(
            "SELECT * FROM user_asset_act WHERE id > 0 AND service_category_id IN (%s)"
            % placeholder,
            con,
            params=args.service_category_ids,
        )
...
def request_update_user_asset_act(
    s,
    csrf_token,
    id_,
    ...
    is_target=None,
    memo=None,
    partner_account_id_hash=None,
    partner_sub_account_id_hash=None,
    partner_act_id=None,
):
    ...
    if partner_sub_account_id_hash:
        params[
            "user_asset_act[partner_sub_account_id_hash]"
...
def update_user_asset_act(s, args):
    ...
    for id_ in ids:
        request_update_user_asset_act(
            ...
            csrf_token,
            id_,
            args.large_category_id,
            args.middle_category_id,
            args.is_target,
            args.memo,
        )
...
def search_category(s, args):
    df = search_category_sub(
        s, args.cache_csv, args.force_update, args.large, args.middle
    )
    print(*df.columns.tolist())
    for index, row in df.iterrows():
        print(*row.tolist())
...
def request_user_asset_acts(s, params={}):
    user_asset_acts = s.get(
        "https://moneyforward.com/sp2/user_asset_acts", params=params
    ).json()
...
def filter_db(s, args):
    ...
    if args.query:
        ...
        flags = update_filter_flags(
            df,
            flags,
            "middle_category",
            args.match_middle_categories,
            args.not_match_middle_categories,
        )
        flags = update_filter_flags(
            df,
            flags,
            "large_category",
            args.match_large_categories,
            args.not_match_large_categories,
        )
        flags = update_filter_flags(
            df,
            flags,
            column_name_for_service_name,
            args.match_service_name,
            args.not_match_service_name,
        )
        flags = update_filter_flags(
            df,
            flags,
            column_name_for_sub_type,
            args.match_sub_account,
            args.not_match_sub_account,
        )
...
def request_transactions_category_bulk_updates(
    s, large_category_id, middle_category_id, ids, sqlite=None, sqlite_table=None
):
    if not ids:
        print("ids is empty")
        return
...
def transactions_category_bulk_updates(s, args):
    ...
    request_transactions_category_bulk_updates(
        s, args.large_category_id, args.middle_category_id, ids
    )
...
setattr(
    ...
    "__exit__",
    lambda self, exc_type, exc_value, traceback: None,
)
...
with add_parser(
    subparsers, "account_summaries", func=get_account_summaries
) as subparser:
...
with add_parser(
    subparsers, "cf_sum_by_sub_account", func=get_cf_sum_by_sub_account
) as subparser:
...
with add_parser(
    subparsers, "cf_term_data_by_sub_account", func=get_term_data_by_sub_account
) as subparser:
    subparser.add_argument("sub_account_id_hash")
...
with add_parser(
    subparsers, "update_enable_transfer", func=update_enable_transfer
) as subparser:
    subparser.add_argument("ids", type=int, nargs="*")


with add_parser(
    subparsers, "update_disable_transfer", func=update_disable_transfer
) as subparser:
    subparser.add_argument("ids", type=int, nargs="*")
...
with subparsers.add_parser("user_asset_acts") as subparser:
    ...
    subparser.add_argument(
        "--is_continuous", type=int, choices=[0, 1]
    )  # 1: from -> "1990-01-01"
    ...
    subparser.add_argument(
        "--list_header", type=str, nargs="+", default=user_asset_acts_list_header
    )
...
with subparsers.add_parser("filter_db") as subparser:
    ...
    with subparser.add_mutually_exclusive_group() as group:
        ...
        group.add_argument(
            ...
            type=int,
            nargs=2,
            metavar=("large_category_id", "middle_category_id"),
        )
    ...
    with subparser.add_mutually_exclusive_group(required=True) as group:
        group.add_argument(
            "-q",
            "--query",
            help="ex) content.notnull() and content.str.match('セブン') and middle_category != 'コンビニ'",
        )
    ...
    with subparser.add_argument_group("group_filter_pattern") as group_filter_pattern:
        ...
        with group_filter_pattern.add_mutually_exclusive_group() as group:
            group.add_argument(
                "-m", "--match_middle_categories", nargs="+", metavar="category"
            )
            group.add_argument(
                "-M", "--not_match_middle_categories", nargs="+", metavar="category"
            )

        with group_filter_pattern.add_mutually_exclusive_group() as group:
            group.add_argument(
                "-l", "--match_large_categories", nargs="+", metavar="category"
            )
            group.add_argument(
                "-L", "--not_match_large_categories", nargs="+", metavar="category"
            )

        with group_filter_pattern.add_mutually_exclusive_group() as group:
            group.add_argument(
                "-s", "--match_service_name", nargs="+", metavar="service_name"
            )
            group.add_argument(
                "-S", "--not_match_service_name", nargs="+", metavar="service_name"
            )

        with group_filter_pattern.add_mutually_exclusive_group() as group:
            group.add_argument(
                "-t", "--match_sub_account", nargs="+", metavar="sub_account"
            )
            group.add_argument(
                "-T", "--not_match_sub_account", nargs="+", metavar="sub_account"
            )

    subparser.add_argument(
        "--cache_category_csv", default="cache_search_categories.csv"
    )
...
with add_parser(
    subparsers, "bulk_update_category", func=bulk_update_category
) as subparser:
